...
type GetDataLoaderModel struct {
	F1 string     `xm:"xls://sheet[5:7]/row[3:x]/col[3]"`
	F2 time.Time  `xm:"xls://sheet[6]/row[:]/col[3] pattern='\\d{4}-\\d{2}-\\d{2}' format='2006-01-02' timezone='Asia/Shanghai'"`
	F3 *time.Time `xm:"xls://sheet[6]/row[:]/col[3] pattern='\\d{4}-\\d{2}-\\d{2}' format='2006-01-02' timezone='Asia/Shanghai'"`
	F4 int        `xm:"xls://sheet[6]/row[:]/col[3] pattern='\\d+'"`
}
